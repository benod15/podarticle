...
export async function getUser(req) {
  ...
  if (!token) {
    return { error: 'Sign in with Google to map a new episode.', status: 401, code: 'AUTH_REQUIRED' };
  }
  const sb = adminClient();
  if (!sb) {
    return { error: 'Sign-in is temporarily unavailable. Please try again shortly.', status: 503, code: 'AUTH_UNAVAILABLE' };
  }
  const { data, error } = await sb.auth.getUser(token);
  if (error || !data?.user) {
    return { error: 'Your session expired — please sign in again.', status: 401, code: 'AUTH_REQUIRED' };
  }
