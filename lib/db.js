...
export async function saveEpisode({ videoId, slug, title, showName, publishedAt, durationSec, transcriptLines, analysis, userId = null, visibility = 'private', category = null }) {
  const client = db();
  if (!client) return null;
  const row = {
    video_id: videoId,
...
const LIBRARY_COLUMNS =
  'video_id, slug, title, show_name, published_at, duration_sec, category, analysis:analysis->summary, top5:analysis->top5, created_at';
...
export async function listEpisodes({ limit = 200, category = null } = {}) {
  const client = db();
  if (!client) return [];
  let query = client
    .from('episodes')
