...
export async function analyzeWithGemini({ title, author, chapters, transcriptLines, apiKey }) {
  ...
  // Free-tier keys get 0 quota on Pro models — fall back to flash-lite
  if (r.status === 429 && primary !== 'gemini-3.1-flash-lite') {
    r = await callModel('gemini-3.1-flash-lite');
  }
  ...
  // Normalize display timestamps from seconds — models sometimes write the
  // `timestamp` field in minute style ("1:19") while seconds stays correct.
  const fmt = (sec) => {
    sec = Math.floor(sec || 0);
    const h = Math.floor(sec / 3600), m = Math.floor((sec % 3600) / 60), s = sec % 60;
    return h ? `${h}:${String(m).padStart(2, '0')}:${String(s).padStart(2, '0')}` : `${m}:${String(s).padStart(2, '0')}`;
  };
  for (const c of analysis.chapters || []) c.timestamp = fmt(c.seconds);
  for (const t of analysis.top5 || []) t.timestamp = fmt(t.seconds);
